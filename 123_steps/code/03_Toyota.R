...
Toyota_by_Day<-readRDS("./data/Toyota/data_Toyota_II.rds")
...
# Toyota_by_Day <- Toyota_by_Day %>% filter(.,date<=as.Date("2016-11-01"))

####### Graph 1 #######################################################################
g1 <- ggplot(Toyota_by_Day, aes(y = Score_Net, x = date)) + 
  geom_line(aes(colour = Score_Net),size=1.5)  + 
  labs(title = "Twitter: 'Toyota'") + 
  ...
  geom_label_repel(data = Toyota_by_Day[Toyota_by_Day$Who=="Toyota",],
...
g2 <- ggplot(data_Toyota, aes(date,score)) 
...
n = max(Toyota_by_Day$Score_Net)

g3 <- ggplot(Toyota_by_Day, aes(x = date)) +
      theme_minimal() +
      ...
      geom_label_repel(data = Toyota_by_Day[!is.na(Toyota_by_Day$Short),],#Toyota_by_Day[samp_ind, ]
...
ggsave("./data/Toyota/Toyota_III.png", arrangeGrob(grobs = plots), width= 50,height = 20,units = "cm")


#View(data_Toyota %>% filter(date > "2016-12-01" & date <"2016-12-30"))