...
Whirpool_by_Day<-readRDS("./data/Whirpool/data_Whirpool_II.rds")
...
NQ_global = sum(data_Whirpool$score>0) # NP = Número de Observaciones Positivas
P = NP_global / N_global
P2 = NP_global / (NP_global - NQ_global)
# Whirpool_by_Day <- Whirpool_by_Day %>% filter(.,date<=as.Date("2016-11-01"))

####### Graph 1 #######################################################################
g1 <- ggplot(Whirpool_by_Day, aes(y = Score_Net, x = date)) + 
  geom_line(aes(colour = Score_Net),size=1.5)  + 
  labs(title = "Twitter: 'Whirpool'") + 
  ...
  geom_label_repel(data = Whirpool_by_Day[Whirpool_by_Day$Who=="Whirpool",],
...
g2 <- ggplot(data_Whirpool, aes(date,score)) 
g2 <- g2 + geom_count(aes(colour = score, alpha =.5), show.legend = F) + #,col = "tomato3"
  theme_minimal() +#$theme_bw()+
...
n = max(Whirpool_by_Day$Score_Net)

g3 <- ggplot(Whirpool_by_Day, aes(x = date)) +
      theme_minimal() +
      ...
      geom_label_repel(data = Whirpool_by_Day[!is.na(Whirpool_by_Day$Short),],#Whirpool_by_Day[samp_ind, ]
...
ggsave("./data/Whirpool/Whirpool_III.png", arrangeGrob(grobs = plots),height = 20,width = 50,units = "cm")

#View(data_Whirpool %>% filter(date > "2016-12-01" & date <"2016-12-30"))