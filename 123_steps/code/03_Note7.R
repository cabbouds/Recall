...
# Note7_by_Day <- Note7_by_Day %>% filter(.,date<=as.Date("2016-11-01"))

####### Graph 1 #######################################################################
g1 <- ggplot(Note7_by_Day, aes(y = Score_Net, x = date)) + 
  geom_line(aes(colour = Score_Net),size=1.5)  + 
  labs(title = "Twitter: 'Note7'") + 
  ...
  geom_label_repel(data = Note7_by_Day[Note7_by_Day$Who=="Note7",],
...
g2 <- ggplot(data_Note7, aes(date,score)) 
...
g3 <- ggplot(Note7_by_Day, aes(x = date)) +
      theme_minimal() +
      scale_color_gradientn(colors = rev(cols), limits = c(0, 1),
      ...
      geom_label_repel(data = Note7_by_Day[!is.na(Note7_by_Day$Short),],#Note7_by_Day[samp_ind, ]
...
ggsave("./data/Note7/Note7_III.png", arrangeGrob(grobs = plots), width= 50, height = 20, units = "cm")

#View(data_Note7 %>% filter(date > "2016-12-01" & date <"2016-12-30"))