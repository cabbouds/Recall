...
Mars_by_Day<-readRDS("./data/Mars/data_Mars_II.rds")
...
# Mars_by_Day <- Mars_by_Day %>% filter(.,date<=as.Date("2016-11-01"))

####### Graph 1 #######################################################################
g1 <- ggplot(Mars_by_Day, aes(y = Score_Net, x = date)) + 
  geom_line(aes(colour = Score_Net),size=1.5)  + 
  labs(title = "Twitter: 'Mars'") + 
  ...
  geom_label_repel(data = Mars_by_Day[Mars_by_Day$Who=="Mars",],
...
g2 <- ggplot(data_Mars, aes(date,score)) 
...
n = max(Mars_by_Day$Score_Net)

g3 <- ggplot(Mars_by_Day, aes(x = date)) +
      theme_minimal() +
      ...
      geom_label_repel(data = Mars_by_Day[!is.na(Mars_by_Day$Short),],#Mars_by_Day[samp_ind, ]
...
ggsave("./data/Mars/Mars_III.png", arrangeGrob(grobs = plots),height = 20,units = "cm")


#View(data_Mars %>% filter(date > "2016-12-01" & date <"2016-12-30"))